...
export default function SuppliersScreen() {
  ...
  const fetchSuppliers = async () => {
    try {
      const { data } = await api.get(`/suppliers?search=${encodeURIComponent(search)}`);
      setSuppliers(data);
    } catch (err) {
  ...
  const handleOpenForm = (supplier?: Supplier) => {
    if (supplier) {
      ...
      setFormCompany(supplier.company || '');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Save Failed', 'Could not save supplier. Please check network connection.');
    } finally {
      setSaving(false);
    }
  };
            ...
            <View key={sup.id} style={styles.card}>
              <View style={styles.cardHeader}>
                <View style={{ flex: 1 }}>
                  ...
                  {sup.company ? (
                    <Text style={styles.cardCompany}>{sup.company}</Text>
                  ) : null}
                </View>
                <View style={styles.actions}>
                  <TouchableOpacity onPress={() => handleOpenForm(sup)} style={styles.actionBtn}>
                    <Feather name="edit" size={16} color={Colors.textSecondary} />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => handleDelete(sup.id)} style={styles.actionBtn}>
              ...
              <View style={styles.details}>
                {sup.phone ? (
                  <View style={styles.detailRow}>
                    ...
                    <Text style={styles.detailText}>{sup.phone}</Text>
                  </View>
                ) : null}
                {sup.email ? (
                  <View style={styles.detailRow}>
                    <Feather name="mail" size={12} color={Colors.textMuted} />
                    <Text style={styles.detailText}>{sup.email}</Text>
                  </View>
                ) : null}
                {sup.address ? (
                  <View style={styles.detailRow}>
                    <Feather name="map-pin" size={12} color={Colors.textMuted} />
                    <Text style={styles.detailText} numberOfLines={1}>{sup.address}</Text>
                  </View>
                ) : null}
              </View>
              ...
              <View style={styles.footer}>
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>{sup._count?.products || 0} Products</Text>
                </View>
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>{sup._count?.purchaseOrders || 0} Orders</Text>
                </View>
              </View>
            </View>
          ))}
